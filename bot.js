...
var dataStorageLocation = "C:/Users/Administrator/Concord_Services/FORCE-Bot/users/"
...
bot.on('message', msg => {
  ...
  timestamp = Number((new Date().getTime() / 1000).toFixed(0));
    ...
    if(command === "frain"){
      if(parameters[0] && Number(parameters[0]) >= 0.1){
		...
		if(msg.guild){								//USER-BASED COMMANDS
			if(command === "fbal"){
				if(!parameters[0] && balance && balance >= 0){
				const embed = {
					"color": 3144381,
					"footer": {
						"icon_url": "https://cdn.discordapp.com/emojis/467170811300675614.png",
    ...
    //if(msg.content.includes("http")){                   //AUTO-MODERATION
